...
- 리액트는 상위에서 해당 하위까지가 복잡해지면 일이 많아지므로 이럴때 Redux가 필요함<br/>리덕스는 모든 데이터(컴포넌트)를 한 곳(스토어)에 몰아두고 그 안에서 데이터들을 가져와 처리해줌<br/>어떠한 컴포넌트는 스토어를 감지해서 일을 처리함<br/>하지만, 스토어에서 관리할것과 하지말아야할 것이 있기때문에 나누기가 힘들어짐
